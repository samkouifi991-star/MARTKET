...
export default async function RiskGaugePage() {
  ...
  const live = demoMode ? null : await getLiveRiskGauge();
  const risk = demoMode ? generateRiskGauge() : live!.result;
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-(--text-faint) mt-1">
          A transparent composite of cross-asset risk signals, from 0 (strong risk-off) to 100 (strong risk-on). Every component and its exact contribution is shown below.
          {!demoMode && " Components with no live data source in the current provider set — volatility index, yield-curve slope, credit spreads — are shown as unavailable rather than estimated."}
        </p>
      </div>

      {!risk ? (
        <Card>
          <p className="text-sm text-(--text-faint) py-6 text-center">{live?.unavailableReason ?? "Real quote data is currently unavailable for this gauge's required components."}</p>
        </Card>
      ) : (
        <>
          {live?.result && (
            <p className="text-xs text-(--text-faint)">
              {live.result.componentsAvailable} of {live.result.componentsTotal} components are live real data today.
            </p>
          )}
          <Card>
